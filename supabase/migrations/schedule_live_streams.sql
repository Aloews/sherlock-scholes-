...
-- задействовано (почему — в шапке функции), поэтому прогон стоит по одному
-- обращению к публичной странице на канал. Десять минут выбраны по
-- содержанию: окно
...
begin
  ...
  select net.http_post(
    ...
    body := '{}'::jsonb,
    -- ⚠️ ЭТИ 50 СЕКУНД — ЖЁСТКАЯ ГРАНИЦА ДЛЯ ФУНКЦИИ, а не запас. Каналы
    -- опрашиваются последовательно, и худший случай (каждый по таймауту)
    -- давно длиннее: двенадцать каналов по 12 секунд — 144. Поэтому в самой
    -- функции стоит собственный бюджет RUN_BUDGET_MS = 40 секунд, и он
    -- ПРИВЯЗАН К ЭТОМУ ЧИСЛУ. Меняете здесь — меняйте и там: иначе ответ с
    -- полем `parsed` перестанет доходить, и поломка разбора станет невидимой.
...
-- что YouTube поменял разметку». Ноль при двенадцати каналах — второе.
-- Поле `skipped` отвечает на другой вопрос: ненулевое значит, что бюджет
-- прогона кончился раньше каналов, и смотреть надо на медленный канал, а
-- не на разбор.
--
-- Снять с расписания:
