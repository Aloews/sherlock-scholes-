...
-- NEWS_LLM_API_KEY/NEWS_LLM_BASE_URL, секреты Supabase. Протокол — Anthropic
-- Messages API (base_url отвечает на /v1/messages, а не /chat/completions),
-- но ключ и бюджет свои, не ANTHROPIC_API_KEY выше. Секретов ещё может не
-- быть — тогда конвейер работает как раньше, просто без этих двух колонок.
--
