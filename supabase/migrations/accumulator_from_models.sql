...
-- есть — 12 543 матча с 22.01.2023. Котировки отвечают на вопрос «выгодно
...
-- подгонка. 167 дней, 8 переобучений, 128–153 билета на правило.
--
-- ЧТО ПОЛУЧИЛОСЬ (доля прошедших экспрессов, три плеча):
--
--     правило                       llm     fly     own
--     топ по уверенности          15.7 %  17.0 %  18.3 %
--     все трое согласны + 0.45    16.9 %  19.4 %  21.3 %
--
-- И на четырёх плечах:
--
--     топ по уверенности          12.2 %  10.9 %  10.2 %
--     все трое согласны + 0.45    14.0 %  12.7 %  13.3 %
--
...
-- (own, 21.3 %, интервал 15.3…28.9) и у базового (18.3 %) интервалы
-- перекрываются. Довод не в величине, а в СОГЛАСОВАННОСТИ: согласие трёх
-- оказалось лучше во всех ШЕСТИ сравнениях (три модели × две длины), ни
-- одного исключения. Будь клетки независимы, это p ≈ 0.03; они не
-- независимы — считаются по одним и тем же дням и часто по тем же матчам, —
-- поэтому это ДОВОД ВЫБРАТЬ, а не доказательство превосходства.
--
-- ⚠️ И ЕЩЁ ОДНО ЧИСЛО, КОТОРОЕ НЕЛЬЗЯ ПРЯТАТЬ. Рядом с фактом стоит
-- ОЖИДАНИЕ — произведение калиброванных вероятностей:
--
--     own  факт 21.3 %  ожидание 15.4 %   лучше ожидания
--     fly  факт 19.4 %  ожидание 15.0 %   лучше ожидания
--     llm  факт 16.9 %  ожидание 19.4 %   ХУЖЕ ожидания
--
  ...
  loop
    foreach v_k in array p_legs loop
      foreach v_model in array array['llm', 'fly', 'own'] loop
        ...
        -- 0.45 по калиброванной вероятности, потом лучшие по вероятности
        -- ЭТОЙ модели. Числа и оговорка про значимость — в шапке файла.
        --
        -- ⚠️ ПОРОГ ЗДЕСЬ НЕ ДЛЯ КРАСОТЫ: он отсекает дни, когда сильных плеч
        -- просто нет. В замере из-за него собиралось 136 билетов вместо 144 —
        -- восемь дней остались БЕЗ экспресса, и это правильный исход.
        ...
        select p.fixture_id, p.pick,
         ...
         where p.model = v_model
           ...
           and calibrated_confidence(p.confidence, p.model) >= coalesce(p_floor, 0.45)
...
-- на боевом бэкфилле `fly` на трёх плечах дал 47.8 %, в ходе вперёд — 19.4 %.

create or replace function public.backfill_model_accumulators(p_floor numeric default 0.45)
returns text
language plpgsql security definer set search_path = public as $$
declare
  v_model text;
