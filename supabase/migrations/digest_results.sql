...
--
-- ⚠️ СОРТИРОВКИ МАЛО — НУЖЕН ОТБОР, И ЭТО ИСПРАВЛЕННАЯ ПОЛОМКА. Первая версия
-- только упорядочивала, ничего не выбрасывая: когда громких матчей в окне НЕТ,
-- шесть первых строк — это то, что нашлось. Замер 03.09.2026 на боевых данных:
-- за 48 часов сыграно 11 матчей, из них громких — НОЛЬ, и сводка каждый раз
-- пересказывала «Мито Холлихок — Касима Антлерс». Владелец увидел это как
-- «постоянный фальшивый матч в краткой сути», и он прав: матч настоящий, но
-- новостью он не был никогда, а стоял там как новость.
--
-- Отбор ровно тот, о котором просили: «результаты громких команд в Лиге
-- чемпионов». УЕФА проходит ВСЕГДА, без оглядки на карточки, — иначе
-- «Ливерпуль — Атлетико» выпадет из-за того, что у «Атлетико» карточки нет
-- вовсе. Всё остальное проходит, только если хотя бы одна сторона знаменита,
-- и порог взят не с потолка: 90 — это FAME_MIN.famous, тот самый, которым
-- в колоде подписаны «знаменитые» (src/shared/types/deck.ts).
--
-- Пустой ответ — НОРМАЛЬНЫЙ ответ. В межсезонье и на паузах сборных громких
-- результатов не бывает, и тогда блок <results> просто не появляется. Молчать
-- правильнее, чем добирать до шести чем попало: сводка без счёта — сводка,
-- сводка с чужим матчем — дезинформация.
-- ===========================================================================
...
language sql stable security definer set search_path = public as $$
  select f.sport_key, f.home_team, f.away_team, f.home_score, f.away_score,
   ...
   where f.completed
     ...
     -- См. шапку: без этой строки сводка добирает до шести чем попало.
     and (f.sport_key like 'soccer_uefa%'
          or greatest(coalesce(hc.fame, 0), coalesce(ac.fame, 0)) >= 90)
