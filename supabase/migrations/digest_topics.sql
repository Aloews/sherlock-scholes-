...
-- стран. Русский к ним не присоединяется: «Torres» и «Торрес» общих основ не
-- имеют. Это ограничение метода, а не недоделка; чтобы его снять, нужен
-- перевод или словарь имён, а не другой порог.
--
...
--   1. Record отдаёт заголовки в CDATA, и обёртка доезжает до базы целиком:
--      «<![CDATA[ Ferran Torres troca ... ]]>». Разбор в football-digest
--      снимает CDATA только когда тег и скобка стоят вплотную.
--   2. Оттуда же приходит битая кодировка: «35 milh?es» вместо «milhões» —
--      лента объявляет одну кодировку, а отдаёт другую.
