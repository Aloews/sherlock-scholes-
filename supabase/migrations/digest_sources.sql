...
insert into public.digest_source (kind, name, ref, lang, enabled, note) values
  ...
  ('feed', 'Sky Sports', 'https://www.skysports.com/rss/12040', 'en', true,
   'Единственная лента с буквенной зоной (BST). Держится на NAMED_ZONES в index.ts — если оттуда убрать BST, источник снова станет молча пустым.')
