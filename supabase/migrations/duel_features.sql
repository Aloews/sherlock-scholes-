...
-- football_scraper/forecast_duel.py — линейная голова, резервуар и свой
-- вариант; итог кладётся в forecast_duel_model / forecast_duel_match и
-- показывается на экране /duel.
--
-- ⚠️ ПРЕЖНИЙ ОДНОРАЗОВЫЙ СКРИПТ (docs/experiments/duel_learning.py) СНЯТ, И НЕ
-- ЗА КОМПАНИЮ. Он выбирал λ по ПРОВЕРОЧНОЙ части — то есть по тем самым
-- матчам, на которых потом отчитывался. Это ровно та ошибка, которую теперь
-- стережёт отдельный случай в check-tests; держать рядом рабочий вариант и
-- образец этой ошибки значит однажды запустить не тот.
--
