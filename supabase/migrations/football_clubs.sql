...
-- ⚠️ joined_at ЗНАЧИТ РАЗНОЕ У РАЗНЫХ ИСТОЧНИКОВ, и в колонке это не видно.
-- Прежний комментарий «дат трансферов у нас нет» устарел ровно наполовину, и
-- половина эта важная: у source = 'wikidata' дата настоящая (P580), у
-- остальных — дата свидетельства, то есть МАТЧА.
--
-- Замер 01.09.2026, из-за которого различие и понадобилось: у wiki_career
-- десять дат, на каждой 20+ игроков; 23.08 «пришли» 29 человек из 14
-- несвязанных клубов — в день, когда сыграно 70 матчей. Тапсоба, Бонифейс и
-- Шик «перешли» в «Байер» в один день, играя там годами.
--
-- Поэтому всё, что говорит о ПЕРЕХОДАХ, обязано фильтровать по source —
-- см. recent_transfers.
comment on table public.club_squad is
  'Состав клуба с историей. ⚠️ joined_at значит РАЗНОЕ по источникам: '
  'у source=''wikidata'' это дата перехода (P580), у остальных — дата '
  'свидетельства (матча). Про переходы читать только wikidata-строки.';
