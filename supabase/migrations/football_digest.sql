...
/**
 * Кириллица и латиница к ОДНОЙ записи.
 *
 * ЗАЧЕМ. Сюжеты сравниваются по общим основам слов, а «Торрес» и «Torres» не
 * имеют ни одной общей буквы. Из-за этого русская половина лент жила отдельно
 * от всех остальных: замер на 826 живых заголовках (391 русский, 435
 * латинских) дал РОВНО НОЛЬ рёбер между алфавитами. Не «мало» — ноль.
 *
 * Виднее всего это было в темах: «Арсенал» — «Манчестер Сити» шёл двумя
 * сюжетами сразу — русским (2 издания, 227 заголовков) и латинским (7
 * изданий), и русский из-за двух изданий стоял ШЕСТЫМ, хотя это главное
 * событие дня. После склейки — одна тема, 9 изданий, первое место.
 *
 * ДВА СВОРАЧИВАНИЯ, И ОБА ВЫБРАНЫ ЗАМЕРОМ, а не звуковым чутьём. Кандидатов
 * проверяли по числу рёбер через алфавиты на тех же сутках:
 *
 *   без сворачивания        10 рёбер   «Барселоны» ≠ Barcelone
 *   c → s                   14         + Торрес, Родри
 *   c → s, j → i            15         + Ювентус
 *   c → s, j → i, y → i     95         ШУМ: склеивает несвязанное
 *   c → k                   12         и Торреса не ловит
 *   двойные буквы, h        10         не меняет ничего
 *
 * Взяты первые два. Все 15 рёбер проверены глазами и верны; ни одно из
 * прежних десяти не потеряно.
 *
 * ЧЕГО ЭТО НЕ УМЕЕТ, и это предел метода, а не недоделка:
 *   * «Гвардиола» ≠ Guardiola — гв против gu, один звук двумя традициями;
 *   * «Комо» ≠ Como — плата за c → s: твёрдое c уходит в s.
 * Каждое следующее правило, снимавшее такие случаи, взрывало шум (см. y → i),
 * а ложная тема здесь хуже пропущенной.
 *
 * ⚠️ ЧТО ЭТА ПРАВКА ВЫТАЩИЛА НА ВИД, И ЭТО НАДО ЗНАТЬ ДО ТОГО, КАК ОТКРОЕШЬ
 * ЭКРАН. Русские ленты публикуют сотни почти одинаковых заметок живого текста
 * («стартовые составы», «онлайн-трансляция»), и они сцепляются транзитивно в
 * один ком: 272 заголовка ОДНИМ кластером — замерено на тех же сутках СТАРОЙ
 * функцией, то есть ком существовал до транслитерации и не ею создан.
 *
 * Но раньше ком стоял ШЕСТЫМ (два русских издания), и его никто не видел.
 * Теперь к нему присоединились 29 латинских заметок про Суперкубок Англии,
 * изданий стало девять — и ком встал ПЕРВЫМ, назвавшись одним случайным
 * матчем РПЛ. То есть верхняя тема утверждает, что девять изданий писали про
 * «Зенит» — «Динамо», а это неправда.
 *
 * Два очевидных лечения проверены и оба отвергнуты замером:
 *   * порог ребра 4 вместо 3 — ком разбирается (262 → 121), но вместе с ним
 *     умирает и склейка через алфавиты: языков в крупнейшем кластере 4 → 1.
 *     Оба живут на одном пороге, разделить их так нельзя;
 *   * называть тему по центроиду, а не по самому раннему заголовку — стоит
 *     всего +5% (770 → 809 мс, а не в 4.4 раза, как показала первая
 *     неудачная запись через коррелированный подзапрос) и даёт подпись
 *     заметно лучше («РПЛ, 4-й тур: „Балтика" уступила „Спартаку", „Зенит"
 *     разгромил „Динамо"»). Но это ЛУЧШАЯ ПОДПИСЬ, А НЕ ПОЧИНКА: число
 *     изданий остаётся тем же и продолжает врать.
 *
 * Настоящее лечение — нетранзитивная кластеризация, и это отдельная работа.
 *
 * ⚠️ СЛОВАРЬ ИМЁН ИЗ КОЛОДЫ ЗДЕСЬ ПРОБОВАЛИ, И ОН ХУЖЕ. У карточек есть и
 * `name`, и `name_en` — готовый выверенный словарь 3710 имён, и соблазн
 * очевиден. Замер: словарь целиком дал 104 ребра, из которых 78 держались
 * ТОЛЬКО на клубе — «Шальке — Реал Мадрид: трансляция» склеивалось с «Luis
 * Llopis dice adiós al Real Madrid». Клуб — не сюжет, он появляется в
 * десятках несвязанных заметок за сутки. Ограничение словаря людьми не спасло:
 * в колоде есть Арсен Захарян, и основа `arsen` совпала с «Арсеналом» — имена
 * людей и клубов сталкиваются в пространстве основ. Не возвращайся к этому
 * без нового замера.
 */
create or replace function public.digest_translit(p_word text)
returns text
language sql
immutable
as $$
  select
    -- 4. j и i — один звук в разных традициях записи: «Ювентус» даёт iuven,
    --    Juventus — juven.
    replace(
      -- 3. Латинское c сворачивается в s: кириллица не различает написаний
      --    Barcelona/Барселона, «с» даёт s, и без этого основы расходятся.
      --    Побочно сходятся и ch/ч: Манчестер и Manchester дают mansh.
      replace(
        translate(
          replace(replace(replace(replace(replace(replace(replace(replace(
            -- 1. Диакритика снимается ЯВНО, а не через unaccent: тот stable,
            --    а эта функция immutable, и вкладывать stable в immutable
            --    значит соврать планировщику. Без этого шага Martínez не
            --    встречается с «Мартинесом», то есть половина трансферов.
            translate(
              lower(p_word),
              'áàâäãåéèêëíìîïóòôöõøúùûüýÿñçšśşžźżčćĝğłđðřňťďůě',
              'aaaaaaeeeeiiiioooooouuuuyyncssszzzccggldd' || 'rntdue'
            ),
            -- 2. Многобуквенные кириллические — ДО однобуквенных, иначе
            --    translate разберёт их поодиночке и склеит не то.
            'ж', 'zh'), 'ч', 'ch'), 'ш', 'sh'), 'щ', 'sch'),
            'ю', 'iu'), 'я', 'ia'), 'ъ', ''), 'ь', ''),
          'абвгдеёзийклмнопрстуфхцыэ',
          'abvgdeeziiklmnoprstufhsye'
        ),
        'c', 's'
      ),
      'j', 'i'
 ...
 *
 * ОСНОВА БЕРЁТСЯ ПОСЛЕ ТРАНСЛИТЕРАЦИИ, а длина проверяется ДО неё. Порядок не
 * произволен: транслитерация меняет длину слова («Ювентус» → iuventus), и
 * порог в четыре буквы, применённый после, отсекал бы другие слова в разных
 * алфавитах — то есть значил бы разное для русского и английского.
 */
create or replace function public.digest_tokens(p_title text)
returns text[]
language sql
immutable
as $$
