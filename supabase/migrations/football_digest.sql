...
--
-- А ЕЩЁ ГРОМКОСТЬ ЗАВИСИТ ОТ ЯЗЫКА СИЛЬНЕЕ, ЧЕМ ОТ ЧИСЛА ИЗДАНИЙ. Первая
-- версия сравнивала слова целиком и давала русскому ровно ноль сгруппированных
-- сюжетов при двух живых источниках: «Зенит» и «Зенита» — разные слова. Теперь
-- сравниваются основы; подробности и замер — в digest_tokens() ниже.
--
 ...
 *
 * СРАВНИВАЮТСЯ ОСНОВЫ, А НЕ СЛОВА ЦЕЛИКОМ, и это не тонкая настройка — без
 * этого громкость по-русски не работает вовсе. Замер на живых данных, 86
 * русских заголовков за сутки от двух изданий:
 *
 *   точное слово   0 сюжетов   ← «Зенит» и «Зенита» — разные токены
 *   первые 5       4 сюжета
 *   первые 4       7 сюжетов
 *
 * Взято 5. Четыре даёт на один верный сюжет больше и один ЛОЖНЫЙ: «Супер-
 * компьютер Opta» и «Суперкубок» склеиваются по обрубку «супе». Значок
 * «громко» утверждает, что об этом написали несколько изданий, и ложное
 * утверждение здесь хуже пропущенного.
 *
 * Английский от этого не меняется (11 сюжетов при любой длине — его слова
 * почти не изменяются с хвоста), испанский растёт с 5 до 11.
 */
...
immutable
as $$
  select coalesce(array_agg(distinct left(w, 5)), '{}')
 ...
 *
 * ⚠️ ЗДЕСЬ РАНЬШЕ БЫЛО НАПИСАНО, ЧТО ПРОСМОТРОВ У ФИДА НЕТ. Есть:
 * media:statistics и media:starRating лежат внутри media:group, бесплатно и
 * без ключа (см. weekend_goals.sql). Утверждение было не проверено, и на нём
 * чуть не построили «лучшие голы» из ничего.
 *
 * Порядок по времени остаётся — но теперь это ВЫБОР, а не отсутствие данных.
 * Раздел отвечает на вопрос «что нового сегодня», а свежий ролик по
 * определению не успел набрать просмотры: рейтинг здесь показывал бы
 * вчерашнее. Лучшее по просмотрам живёт в digest_weekend_goals(), где окно
 * закрыто и числа успели устояться.
 */
