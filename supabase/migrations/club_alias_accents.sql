...
-- ---------------------------------------------------------------------------
-- ⚠️ ЭТИ ЧЕТЫРЕ СТРОКИ ЖИЛИ БЫ ДО ПЕРВОГО НОЧНОГО ПЕРЕСБОРА.
--
-- `build_club_aliases()` начинается с `delete from club_alias` и собирает
-- таблицу заново из `club_alias_seed` и выводимых правил. Всё, вписанное в
-- `club_alias` руками, она стирает молча — и ночной `rebuild_clubs_all`
-- стёр бы ровно то, что чинил этот файл.
--
-- Поэтому те же четыре соответствия перенесены в `club_alias_seed` —
-- см. `squads_everywhere_value.sql`. Здесь они оставлены как есть: файл
-- описывает, ЧТО было сломано, и переписывать историю ради опрятности
-- значит потерять причину.
-- ---------------------------------------------------------------------------
