...
--
-- ⚠️ ЭТОТ ФАЙЛ БОЛЬШЕ НЕ ИСТОЧНИК ПРАВДЫ ДЛЯ apply_espn_crests.
-- Действующее определение — supabase/migrations/espn_crests_match_once.sql.
-- Отбор ниже соединяет по `club_match_key(...) = club_match_key(...)`, то
-- есть зовёт нормализатор ТРИ МИЛЛИОНА раз и падает с 57014 statement
-- timeout уже на 200 именах (замер 04.09.2026) — ночной шаг падал бы каждую
-- ночь. И `order by length(espn_name)` там выдаёт клубу ЧУЖОЙ герб:
-- «Vitória S.C.» получал эмблему бразильского EC Vitória.
-- Применять этот файл поверх нового значит вернуть обе поломки.
--
-- Там же — поправка к обещанию «587 клубов закроются сами»: закрылось 30.
-- У 514 из оставшихся 580 нет латинского имени вовсе, и соединять их с
-- любым иностранным источником не по чему.
-- ===========================================================================
