 ...
 *
 * Без ключа конвейер работает как раньше: колонки просто не заполняются,
 * ничего не падает.
 *
 * ⚠️ АДРЕС И МОДЕЛЬ — С РАБОЧИМИ УМОЛЧАНИЯМИ, И ЭТО ПОЧИНКА, А НЕ УДОБСТВО.
 * Секретов было три, и незаданным оставался ХОТЯ БЫ ОДИН: `summary_short` пуст
 * у ВСЕХ 2944 заметок, то есть пересказ не собирался ни разу за всё время.
 * Ровно тот же изъян уже лечили у digest-summary — там об этом написано
 * прямо: «забыть один из трёх — обычное дело, и тогда фича молча остаётся
 * сломанной по той же причине, из-за которой всё это и затевалось».
 *
 * Умолчания взяты не из головы: владелец сказал про этот шлюз «только она
 * работает», и digest-summary через него отвечает в бою (`gateway/claude-opus-5`).
 * Теперь включение стоит ОДНОГО секрета — NEWS_LLM_API_KEY.
 *
 * ⚠️ КЛЮЧ ПАДАЕТ НА SUMMARY_LLM_API_KEY, И ЭТО РЕШЕНИЕ ВЛАДЕЛЬЦА, А НЕ МОЁ.
 *
 * Я держал это отдельно и не включал сам: бюджеты разделены по причине,
 * названной выше — здесь разбирается КАЖДАЯ новая заметка и КАЖДЫЙ ролик, а у
 * сводки восемь тем по нажатию. Посадить одно на бюджет другого — это потратить
 * чужие деньги, и решать такое не мне.
 *
 * Владелец решил прямо: «сейчас нейронка используется в краткой сути, используй
 * её же». Поэтому свой ключ по-прежнему ПРЕДПОЧИТАЕТСЯ, если задан, а запасным
 * идёт ключ сводки — тот самый шлюз, про который сказано «только она работает».
 *
 * Что это меняет по расходу, чтобы это не было сюрпризом: 2944 заметки без
...
const NEWS_LLM_BASE_URL = Deno.env.get("NEWS_LLM_BASE_URL")
  ?? Deno.env.get("SUMMARY_LLM_BASE_URL")
  ?? "https://ai.starimg.ru";
const NEWS_LLM_MODEL = Deno.env.get("NEWS_LLM_MODEL")
  ?? Deno.env.get("SUMMARY_LLM_MODEL")
  ?? "claude-opus-5";
 ...
 *
 * Шестьдесят секунд, а не двадцать: внутри есть вызов модели, и у него
 * бывает законная минута. Срок тут — потолок против зависания, а не
 * норматив скорости.
 */
...
interface Source {
  ...
  lang: string | null;
  needs_key: boolean;
  /** 0 — опрашивать каждый прогон, 1..6 — раз в час. См. pollGroupNow. */
  poll_group: number;
}
 ...
 */
/**
 * Какая группа клубов опрашивается в этот прогон: 1..6.
 *
 * Считается ОТ ЧАСОВ И МИНУТ, а не от счётчика в базе: прогон должен быть
 * воспроизводим и не зависеть от того, сколько раз его запускали руками.
 * Шесть групп на шесть прогонов в час дают каждому клубу ровно один опрос в
 * час, и группа меняется на каждом десятиминутном шаге.
 *
 * ⚠️ Привязано к расписанию каждые 10 минут (schedule_football_digest.sql).
 * Сменится период — сменится и смысл: на двадцатиминутном шаге группа станет
 * повторяться через одну, и половина клубов замолчит. Расписание и делитель
 * ниже менять только вместе.
 *
 * (Про cron-выражения в этом комментарии написано словами намеренно: звёздочка
 * со слешем закрывает блочный комментарий и ломает файл.)
 */
function pollGroupNow(): number {
  return Math.floor(new Date().getUTCMinutes() / 10) % 6 + 1;
}

async function loadSources(): Promise<Source[] | null> {
  const r = await fetch(
    `${SUPABASE_URL}/rest/v1/digest_source` +
      `?select=kind,name,ref,lang,needs_key,poll_group&enabled=is.true&order=kind,id`,
    {
      headers: {
        apikey: SERVICE_ROLE,
...
async function run(useLlm: boolean): Promise<Response> {
  ...
  //
  // ⚠️ И РОТАЦИЯ ПО poll_group — ОНА ДЕРЖИТ КВОТУ. Клубных каналов 47, лиг 9,
  // на канал уходит ДВЕ единицы (playlistItems + videos.list), прогон идёт
  // 144 раза в сутки. Опрашивать всех каждый прогон — 16 128 единиц, при
  // суточной квоте 10 000. Поэтому лиги (группа 0) идут всегда, а клубы
  // разложены по шести группам, и за прогон берётся ровно одна: каждый клуб
  // опрашивается раз в час, расход держится на 4 896 единицах (49%).
  //
  ...
  const channels = sources.filter((s) =>
    s.kind === "channel" &&
    (YT_KEY !== "" || !s.needs_key) &&
    // ⚠️ `?? 0` НЕ КОСМЕТИКА. Функция и миграция выкатываются порознь, и если
    // функция уедет первой, столбца ещё нет — `poll_group` придёт undefined.
    // Без запасного нуля `undefined === 0` ложно, `undefined === group` тоже,
    // и тогда НИ ОДИН канал не опрашивается: дайджест замолкает целиком и
    // молча, а по отчёту это выглядит как «каналы ничего не дали».
    ((s.poll_group ?? 0) === 0 || s.poll_group === group)
  );
