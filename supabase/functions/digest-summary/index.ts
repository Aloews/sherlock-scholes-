...
function systemPrompt(langName: string): string {
  return [
    ...
    "",
    "Отдельно тебе дают блок <results> — СЫГРАННЫЕ МАТЧИ. Это наши собственные",
    "данные из расписания, а не чужой текст: кто кого обыграл и где была ничья.",
    "",
    "⚠️ СЧЁТ НЕ ПИШИ НИКОГДА. Ни из блока, ни из заголовков, ни по памяти:",
    "никаких «2:1», «обыграл со счётом 3:0», никаких цифр через двоеточие или",
    "тире. Счёта в блоке нет вовсе, и это не упущение — так решено. Пиши",
    "исходом: «обыграл», «уступил», «сыграли вничью», «прошёл дальше».",
    "",
    ...
    "не быть вовсе, и тогда пиши сводку по одним темам.",
    "",
    "Чего не делать:",
    "— не выдумывай подробностей, которых нет в заголовках: суммы, даты, имена.",
    "  Ты видишь только заголовки, читатель проверить не сможет и просто",
    "  поверит — поэтому лучше сказать меньше и точно;",
    "— не приписывай матчу из <results> ничего сверх исхода: ни счёта, ни голов,",
    "  ни имён, ни удалений. В блоке ровно две команды и кто кого обыграл;",
...
/**
 * Матч в подсказке — ИСХОДОМ, А НЕ СЧЁТОМ.
 *
 * ⚠️ ВЛАДЕЛЕЦ: «счета не пиши никогда». Прямое указание после того, как модель
 * дописала «Челси обыграл Реал Сосьедад 1:0» — матч, которого в блоке не было.
 *
 * Поэтому чисел модель больше НЕ ВИДИТ ВОВСЕ. Запретить писать счёт, продолжая
 * его показывать, — это надежда на послушание; убрать числа из входа — это
 * свойство. Сюжет от этого не теряется: «победа Реала над Интером» — тот же
 * факт из той же нашей таблицы, просто без цифры, которую читатель всё равно
 * не мог проверить.
 *
 * Счёт при этом остаётся в digest_results и в ключе кэша: новый счёт обязан
 * пересобирать сводку, он просто не доезжает до подсказки.
 *
 * Пометка [УЕФА] — то, по чему модель отличает главное от рядового тура. Без
 * неё японский тур открывал сводку наравне с Лигой чемпионов: свежесть модель
 * видит, а важность — нет.
 */
function outcomeLine(r: MatchResult): string {
  const head = `${r.is_uefa ? "[УЕФА] " : ""}${tournamentName(r.sport_key)}: `;
  if (r.home_score === r.away_score) {
    return `${head}ничья, ${r.home_team} — ${r.away_team}`;
  }
  const winner = r.home_score > r.away_score ? r.home_team : r.away_team;
  const loser = r.home_score > r.away_score ? r.away_team : r.home_team;
  return `${head}победа ${winner} над ${loser}`;
}

/**
 * Счёт в готовом тексте — и его удаление.
 *
 * ⚠️ ЭТО ВТОРОЙ РУБЕЖ, И ОН ЗДЕСЬ ИМЕННО ПОТОМУ, ЧТО ПЕРВЫЙ УЖЕ ПОДВОДИЛ.
 * Подсказка запрещала выдумывать счёт — модель всё равно вывела его из
 * заголовка. Запрет, исполнение которого нельзя проверить, не запрет.
 * Здесь проверка есть: что бы модель ни написала, цифровой счёт до игрока
 * не доедет.
 *
 * Шаблон намеренно узкий — ОДНА цифра, разделитель, ОДНА цифра, на границах
 * слова. Иначе он съел бы «19:00» (время) и «2024–2025» (сезон): в «19:00»
 * перед 9 нет границы слова, в «2024–2025» перед 4 тоже — поэтому оба целы.
 * Футбольный счёт двузначным практически не бывает.
 *
 * Чего он НЕ ловит: счёт словами («два-ноль»). Это не тот риск: выдуманная
 * цифра выглядит как факт, выдуманная пропись — как оборот речи.
 */
const SCORE_PATTERN = String.raw`\s*(?:со\s+сч[её]том\s*)?\b\d\s*[:\-–]\s*\d\b`;

function hasScore(text: string): boolean {
  return new RegExp(SCORE_PATTERN).test(text);
}
...
function userPrompt(topics: Topic[], results: MatchResult[]): string {
  ...
  return [
    ...
    results.length === 0
      ...
      : "Напиши сводку. Счёт не указывай нигде и ни у одного матча — только "
        + "исход.",
  ].join("\n");
}
...
Deno.serve(async (req) => {
  ...
  // ⚠️ ПОСЛЕДНЯЯ ПРОВЕРКА ПЕРЕД ИГРОКОМ, И ОНА НЕ СПРАШИВАЕТ МОДЕЛЬ.
  // «Счета не пиши никогда» — требование владельца, а не пожелание. Подсказка
  // выше просит, здесь проверяется: если цифровой счёт всё же появился, он
  // вырезается из текста, а в логах остаётся след — значит подсказку обошли и
  // это надо разбирать, а не терпеть.
  const text = hasScore(answer.text)
    ? (console.error(`score leaked into summary (${lang}), stripped`),
       stripScores(answer.text))
    : answer.text;
