// ⚠️ `EdgeRuntime` ДАЁТ РАНТАЙМ SUPABASE, А НЕ DENO, И БЕЗ ЭТОГО ОБЪЯВЛЕНИЯ
// ЭТО ПРОСТО НЕИЗВЕСТНОЕ ИМЯ. `deno check` валился на TS2304, и заметить это
// было негде: типы Edge-функций до сих пор не проверял никто — ни CI, ни
// кнопка выкладки. Объявление стирается при сборке, то есть в рантайме
// используется настоящий глобал; здесь оно нужно ровно затем, чтобы
// проверка типов видела контракт, на который мы опираемся.
//
// Ссылка `/// <reference types="jsr:@supabase/functions-js/edge-runtime.d.ts" />`
// выглядит уместнее, но `jsr:` в тройной ссылке не разрешается — проверено,
// TS2304 остаётся.
