...
# --- дата перехода --------------------------------------------------------
# ⚠️ ГЛАВНАЯ ПРОВЕРКА, ДОБАВЛЕННАЯ ПО ЖИВОЙ ПОЛОМКЕ. Сборщик шёл час, дошёл
# до 975-го игрока из 8972 и умер: Transfermarkt отдал «0000-00-00» вместо
# неизвестной даты, Postgres ответил «date/time field value out of range», и
# вся пачка не записалась. Пустая строка значит «дата неизвестна» — это
# правда; выдуманный год был бы ложью.
check(tr.parse_date("0000-00-00") == "", "заглушка источника — не дата")
check(tr.parse_date("2026-07-09") == "2026-07-09", "настоящая дата проходит")
check(tr.parse_date("2026-00-05") == "", "нулевой месяц отвергнут")
check(tr.parse_date("2026-13-01") == "", "тринадцатый месяц отвергнут")
check(tr.parse_date("2026-01-00") == "", "нулевой день отвергнут")
check(tr.parse_date("1800-01-01") == "", "год до футбола отвергнут")
check(tr.parse_date("01.02.2020") == "", "другой формат не угадываем")
check(tr.parse_date(None) == "" and tr.parse_date("") == "", "пусто — пусто")

# И то же самое через разбор целиком: заглушка не должна дойти до записи.
bad = tr.parse_transfers({"transfers": [{
    "url": "/x/transfer_id/1", "dateUnformatted": "0000-00-00",
    "from": {"clubName": "A"}, "to": {"clubName": "B"}}]})
check(len(bad) == 1 and bad[0]["moved_on"] == "",
      "заглушка не доходит до записи, а сам переход сохраняется")
