...
def test_guess_order():
    ...
    # ⚠️ И СРЕДИ НЕПРОБОВАННЫХ — СНАЧАЛА ИЗВЕСТНЫЕ. Без этого ключа у всех
    # непробованных ключ одинаков — (-1, ""), — сортировка устойчива, и
    # порядок остаётся тем, в котором карточки пришли из базы: по `id`.
    # Бюджет кончается на первой тысяче из двадцати с лишним, так что кто в
    # неё попадёт, решал номер карточки. Замер на боевой базе 15.09.2026: у
    # ВСЕХ тридцати карточек с известностью 70+ и без статистики `tried = 0`
    # — Тьерри Анри (100), Родриго (97), Агуэро (96), Карвахаль (95).
    famous = [_card("a", "А", 10.0), _card("b", "Б", None),
              _card("c", "В", 100.0), _card("d", "Г", 90.0)]
    byfame = [c["id"] for c in guess_order(famous, wanted, set(), {})]
    ok &= check("непробованные: сначала известные", byfame, ["c", "d", "a", "b"])

    # Известность НЕ перебивает очередь: пробованный знаменитый всё равно
    # уступает непробованному безвестному, иначе очередь снова встанет.
    misses = {"c": (1, "2026-09-11")}
    byfame = [c["id"] for c in guess_order(famous, wanted, set(), misses)]
    ok &= check("пробованный знаменитый уступает непробованному",
                byfame, ["d", "a", "b", "c"])

    # ОТРИЦАТЕЛЬНЫЙ КОНТРОЛЬ: порядок обязан ЗАВИСЕТЬ от известности. Те же
    # карточки с одинаковой известностью дают исходный порядок — значит
    # проверка выше поймала бы возврат к сортировке по `id`.
    flat = [_card("a", "А", 5.0), _card("b", "Б", 5.0),
            _card("c", "В", 5.0), _card("d", "Г", 5.0)]
