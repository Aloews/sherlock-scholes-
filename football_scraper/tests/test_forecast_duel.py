...
# ⚠️ ГЛАВНАЯ ЗАЩИТА ОТ ПОДГЛЯДЫВАНИЯ. Подбор по той же выборке, на которой
# учились, всегда выбирает МЕНЬШИЙ штраф: без штрафа обучающая подгоняется
# лучше всего. Разойдись эти два числа — значит λ выбирается не там, где надо.
_, lam_on_train = F.fit_linear(Xn[tn], yn[tn], Xn[tn], yn[tn])
check("подбор по ОТЛОЖЕННОЙ части штрафует сильнее, чем подбор по обучающей",
      lam_n > lam_on_train,
      f"отложенная {lam_n}, обучающая {lam_on_train} — если равны, "
      "подбор идёт по тем же матчам, на которых потом отчитываются")
...
# Двадцать матчей: два далеко от порога и угаданы, восемнадцать впритык и
# угаданы наполовину. Запас 9.0 дал бы 100 % на десятой части матчей.
# ⚠️ min_coverage НЕ ПЕРЕДАЁТСЯ НАРОЧНО: проверяется сама постоянная. С
# явным значением тест прошёл бы и при снятой защите — так и вышло в первый
# раз, и check-tests назвал его ПУСТЫМ.
p_far = [12.0, 12.0] + [2.6, 2.4] * 9
y_far = [9.0, 9.0] + [9.0, 9.0, 9.0, 0.0] * 4 + [9.0, 0.0]
few = F.fit_gate(p_far, p_far, y_far, margins=(0.0, 9.0))
...
check("на этих же данных снятая защита ВЫБРАЛА БЫ молчание",
      F.fit_gate(p_far, p_far, y_far, margins=(0.0, 9.0), min_coverage=0.0) == 9.0,
      "если и без защиты выбирается ноль, случай выше ничего не проверяет")
