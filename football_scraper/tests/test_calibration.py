...
# ⚠️ И ОТДЕЛЬНО — НАКЛОН ОБЯЗАН БЫТЬ РОВНО НУЛЁМ, а не «каким-нибудь».
#
# Это не придирка к красоте числа. Две проверки выше смотрят на ОТВЕТ в точке
# 0.90 — а он выйдет верным при любом (a, b) на гребне: при вырожденном входе
# слагаемое a·logit(p) одинаково у всех точек, и подгонка может размазать его
# между a и b как угодно. Проверка проверок это и показала: убрав защиту, обе
# строки выше остались зелёными — шаг с дроблением не даёт Ньютону разойтись,
# и Brier получается приличный.
#
# Опасность в другом. Выдуманный наклон СОХРАНЯЕТСЯ в базу и применяется к
# БУДУЩИМ прогнозам, у которых уверенность уже разная. Наклон, выученный там,
# где различать было нечем, начнёт растягивать и сжимать чужие числа — и вот
# это уже не видно ни в одном Brier, посчитанном на старой выборке.
#
# Правильный ответ известен заранее: различать нечем — наклона нет.
degenerate = [(0.7, 1.0 if rng.random() < 0.3 else 0.0) for _ in range(500)]
a_d, b_d = fit_platt(degenerate)
check(f"вырожденный вход: наклон ровно 0, а не {a_d:.4f}", a_d == 0.0)
check("вырожденный вход: сдвиг даёт долю попаданий",
      abs(sigmoid(b_d) - sum(y for _, y in degenerate) / len(degenerate)) < 0.02)
check("вырожденный вход: две подгонки подряд дают одно и то же",
      fit_platt(degenerate) == (a_d, b_d))
