...
function TrainingGame({ filter, onPlayAgain }: TrainingGameProps) {
  ...
  const clubChips = (entry: HistoryEntry): string[] => {
    ...
    // ⚠️ ТЕКУЩИЙ КЛУБ ИДЁТ ПЕРВЫМ И НЕ ЗАМЕНЯЕТ КАРЬЕРУ. Открытый период в
    // статье — часть настоящей карьеры, выкинуть его нельзя; выдать его за
    // сегодня тоже нельзя. Поэтому собранный текущий клуб стоит отдельной
    // строкой выше, а список карьеры остаётся списком карьеры.
    //
    // ⚠️ ПОДПИСИ «СЕЙЧАС» ЗДЕСЬ БОЛЬШЕ НЕТ, и различать строки она не
    // помогала: форму задаёт САМА строка. Карьерная несёт годы и числа
    // («Арсенал 2006–18 · 270 матчей»), текущий клуб — голое имя. Слово
    // повторяло то, что и так видно, и занимало начало каждой первой строки.
    const now = clubNow.get(entry.id)?.club;
    if (now) {
      const clean = cleanClub(now);
      if (clean) out.unshift(clean);
    }
    return out;
  };
