  ...
  return (
    <div className="fixed inset-0 z-50 bg-brand-bg ds-screen overflow-y-auto animate-slide-up">
      ...
      <div className="max-w-sm mx-auto px-5 py-5 space-y-5 pb-12">
        ...
        {/* ⚠️ ВТОРАЯ ИСТОРИЯ КЛУБОВ — ТОЛЬКО ТАМ, ГДЕ ПЕРВОЙ НЕТ. Владелец:
            «в карточке действующих игроков история клубов написана два раза,
            первый точнее, а второй блок с историей собирался с другого
            источника и он не точный (но его можно оставить у игроков, которые
            закончили карьеру)».

            Первая — блок выше: сезоны с Transfermarkt, с матчами, голами и
            минутами по каждому клубу. Вторая — из инфобокса Википедии
            (`career_stats`) или из Викиданных у легенды (`legend_career`): там
            клуб бывает назван как попало и период указан приблизительно.

            ⚠️ УСЛОВИЕ НЕ «ИГРОК ДЕЙСТВУЮЩИЙ», А «ТОЧНАЯ ИСТОРИЯ ЕСТЬ И ОН
            ДЕЙСТВУЮЩИЙ» — обе половины нужны. Действующий игрок, которого мы
            не собрали по сезонам, остался бы вовсе без клубной истории:
            прятать единственное, что есть, ради борьбы с дублем — это менять
            дубль на пустоту. Закончившим карьеру блок оставлен, как и
            просили: у них он часто единственный. */}
        {career.length > 0 && !(club && totals && totals.club_apps > 0) && (
          <Section title={t('collection.career')}>
            <div>
              {career.map((row, i) => {
                ...
                // ⚠️ ССЫЛКА ВЕДЁТ НА ЭКРАН КОМАНДЫ, А НЕ В КАРТОЧКУ КЛУБА.
                // Владелец: «при нажатии на клуб на всех экранах ссылайся на
                // „команды и статистика“». Карточка клуба это имя и герб;
                // экран команды — состав со стоимостями, матчи и тренер.
                // И ключ есть у клуба без карточки тоже, а карточка — у 430
                // клубов из 2097: условие на `card_id` отсекало большинство
                // строк карьеры от единственного места, где о клубе
                // рассказано.
                return found?.club_key ? (
                  <button
                    key={`${row.club}-${i}`}
                    type="button"
                    onClick={() => {
                      hapticImpact('light');
                      navigate(`/club/${encodeURIComponent(found.club_key!)}`);
                    }}
