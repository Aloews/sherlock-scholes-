    ...
    <div className="min-h-screen bg-brand-bg ds-screen text-white p-4 max-w-2xl mx-auto space-y-4">
      ...
      {tab === 'forecast' && isAdmin && (
        <div className="space-y-4">
          ...
          {/* ⚠️ ВХОД В «КТО ЛУЧШЕ ПРЕДСКАЗЫВАЕТ» ПЕРЕЕХАЛ СЮДА С ЭКРАНА МАТЧЕЙ.
              Владелец убрал его оттуда: на «ближайшие матчи» приходят ставить
              свой прогноз, а замер трёх моделей — вопрос этой вкладки. Экран
              не удалён, и ссылка здесь ровно затем, чтобы `/duel` не остался
              без единого входа: маршрут, в который нельзя попасть, ничем не
              отличается от удалённого, но выглядит живым. */}
          <button
            type="button"
            onClick={() => navigate('/duel')}
            className="text-[12px] text-brand-accent"
          >
            Кто лучше предсказывает — замер трёх моделей →
          </button>
        </div>
      )}
