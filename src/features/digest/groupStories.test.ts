...
describe('groupStories', () => {
  ...
  // Раскрытию сюжета нужны САМИ ЗАМЕТКИ, а не названия изданий: оно читает их
  // тексты. Ведущая входит в состав — иначе вызывающему пришлось бы склеивать
  // её обратно и гадать, не попала ли она туда дважды.
  it('отдаёт все заметки сюжета, включая ведущую', () => {
    const [story] = groupStories(DELAP, 'en');
    expect(story.items).toHaveLength(4);
    expect(story.items).toContain(story.lead);
    expect(new Set(story.items.map((n) => n.source)).size).toBe(4);
  });
