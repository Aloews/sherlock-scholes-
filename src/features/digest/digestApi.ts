...
export interface NewsItem {
  ...
  /**
   * СУТЬ ДЛЯ ЭКРАНА, и рисовать надо именно её: пересказ модели, если он есть,
   * иначе начало самой статьи. Выбор делает `digest_news()`, чтобы три
   * компонента не разошлись в том, что считать сутью.
   *
   * ⚠️ СЕГОДНЯ ОНА NULL У ВСЕХ, И ЭТО НЕ ОШИБКА КЛИЕНТА. Пересказ не пишется
   * ни одной заметке — секреты NEWS_LLM_* не заданы (0 сутей на 881 свежую
   * заметку, замер 30.08.2026), — а текст статьи начнёт сохраняться только
   * после деплоя football-digest. До этого лента выглядит как выглядела:
   * строки просто нет, и это норма вёрстки, а не дыра.
   */
...
export interface LiveMatch {
  ...
  seen_at: string;
  /**
   * Когда эфир НАЧАЛСЯ на самом деле. Приходит из `actualStartTime` YouTube,
   * то есть это факт, а не расписание.
   *
   * ⚠️ ЭТОГО ПОЛЯ ЗДЕСЬ НЕ БЫЛО, И ИМЕННО ПОЭТОМУ ЭКРАН ВРАЛ. «Идёт сейчас»
   * значило «конвейер видел строку час назад», а не «эфир идёт»: в разделе
   * висели анонсы, вплоть до назначенного на послезавтра. Теперь `null` сюда
   * не приходит — `digest_live_matches` такие строки не отдаёт, — но тип
   * остаётся обязательным, чтобы забыть про него было нельзя.
   */
  started_at: string;
}
...
/**
 * Эфир, который ещё НЕ НАЧАЛСЯ, но начнётся сегодня.
 *
 * ⚠️ ЗАЧЕМ ОТДЕЛЬНЫЙ СПИСОК, А НЕ ПРОСТО ВЫБРОСИТЬ АНОНСЫ. Выбросить значило
 * бы опустошить экран: почти всё, на что указывает страница канала, ещё не
 * началось. Врала ПОДПИСЬ «идёт сейчас», а не сама трансляция — анонс под
 * честной подписью «начало в 20:00» полезен, и владелец просил не убрать
 * трансляции, а перестать называть идущим то, что не идёт.
 */
export interface UpcomingMatch {
  video_id: string;
  channel: string;
  title: string;
  /** Назначенное начало — `scheduledStartTime` YouTube. */
  scheduled_start_at: string;
}

export async function fetchUpcomingMatches(limit = 6, hours = 12): Promise<UpcomingMatch[]> {
  const { data, error } = await supabase.rpc('digest_upcoming_matches', {
    p_limit: limit,
    p_hours: hours,
  });
  if (error) {
    console.error('[digest] digest_upcoming_matches failed:', error.code, error.message);
    return [];
  }
