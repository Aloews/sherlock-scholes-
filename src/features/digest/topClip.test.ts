...
vi.mock('./digestApi', () => ({
  fetchWeekendGoals: vi.fn(),
  fetchGoals: vi.fn(),
}));

import { fetchWeekendGoals, fetchGoals } from './digestApi';
...
const weekend = (over: Partial<Record<string, unknown>> = {}) => ({
  ...
  weekend_start: '2026-08-08T00:00:00Z', weekend_end: '2026-08-10T00:00:00Z',
  ...over,
});
...
const mockWeekend = vi.mocked(fetchWeekendGoals);
const mockDaily = vi.mocked(fetchGoals);

beforeEach(() => {
  mockWeekend.mockReset();
  mockDaily.mockReset();
});
...
describe('fetchTopClip', () => {
  ...
  it('просит у сервера ровно один ролик', async () => {
    mockWeekend.mockResolvedValue([weekend()] as never);
    await fetchTopClip();
  ...
  it('называет голом только то, что сервер признал голом', async () => {
    mockWeekend.mockResolvedValue([weekend({ is_goal: true })] as never);
  ...
  it('откатывается на свежее видео, когда выходных ещё нет', async () => {
    ...
    mockWeekend.mockResolvedValue([] as never);
