...
/**
 * Сколько матчей просит экран.
 *
 * ⚠️ ЭТО ЧИСЛО ДЕРЖИТ НЕ ДЛИНУ СПИСКА, А ГЛУБИНУ ОКНА, И ОДИН РАЗ ОНО УЖЕ
 * ПРОСЕЛО МОЛЧА. Пока сборных в расписании не было, 60 строк накрывали около
 * девяти суток. С матчами сборных плотность выросла вдвое, и те же 60 строк
 * стали накрывать 4,5 суток — чип «Ла Лига» показывал бы меньше туров, чем
 * накануне, без единой ошибки на экране.
 *
 * Замер 22.09.2026 по боевой базе: 60 → 4,5 суток, 90 → 5,2, 120 → 6,5,
 * 150 → 9,5. Взято 120: возвращает прежний горизонт, оставаясь вчетверо ниже
 * потолка PostgREST (`db-max-rows` = 1000, см. scripts/check-limits.mjs).
 */
export const UPCOMING_LIMIT = 120;
