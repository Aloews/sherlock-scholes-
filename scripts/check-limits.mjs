...
// ------------------------------------------------------------- бандлы -------
// --------------------------------------- котировки: далеко ли ближайшие ----
//
// ⚠️ ЭТОТ ЗАМЕР ЗАВЕДЁН ПО ЖИВОЙ ЖАЛОБЕ, И ОН РОВНО ТОТ, КОТОРОГО НЕ ХВАТАЛО.
// 21.09.2026 `check-prod` показывал по котировкам полную зелень — 3212 строк,
// свежие, 106 матчей, сбор час назад, — а панель экспресса не собирала НИ
// ОДНОГО билета и советовала «понизьте порог». Оба были правы: котировки
// есть, но ВСЕ на матчи с 9 октября. У десяти купленных лиг перерыв на
// сборные, а ближайшие две недели заняты сборными, МЛС и Аргентиной — то
// есть в окне 72 часа матчей с котировками НОЛЬ при любом пороге.
//
// Здоровье сбора и пригодность котировок — разные вопросы, и первый зеленел
// при мёртвом втором. Поэтому число печатается здесь, а не проверяется в
// check-prod: на перерыве сборных ноль в окне НОРМАЛЕН, падать тут нечему.
// Смотреть стоит на `ближайший с котировками`: если он дальше, чем через
// неделю, экспресс за «3 дня» будет пустым, и это не поломка.
async function oddsWindow() {
  const url = env('VITE_SUPABASE_URL');
  const svc = env('SUPABASE_SERVICE_KEY') || env('SUPABASE_KEY');
  if (!url || !svc) {
    row('Котировки: окно экспресса', 'не измерено', 'нет SUPABASE_KEY', 'skip');
    return;
  }
  try {
    // ⚠️ ЧЕРЕЗ ФУНКЦИЮ, А НЕ ЗАПРОСОМ К ТАБЛИЦЕ. У `fixture_odds` намеренно
    // нет грантов вообще — её не читает даже service_role, и это работающая
    // защита §4.4, а не помеха. `odds_health` отдаёт числа и ни одной цены.
    const r = await fetch(`${url}/rest/v1/rpc/odds_health`, {
      method: 'POST',
      headers: { apikey: svc, Authorization: `Bearer ${svc}`,
                 'Content-Type': 'application/json' },
      body: '{}',
    });
    const h = r.ok ? (await r.json().catch(() => []))[0] ?? {} : {};
    const next = h.next_priced ? new Date(h.next_priced) : null;
    const days = next ? (next - Date.now()) / 86400000 : null;

    row('Котировки: матчей в ближайшие 72 ч', `${h.priced_72h ?? '?'}`,
        Number(h.priced_72h ?? 0) > 0
          ? 'экспресс за «3 дня» соберётся'
          : '⚠ экспресс за «3 дня» будет пуст — панель это скажет и предложит '
            + 'окно шире, но САМА НЕ РАСШИРИТ (выбор игрока — закон)',
        Number(h.priced_72h ?? 0) > 0 ? 'ok' : 'warn');

    row('Котировки: ближайший матч с ценой',
        next ? next.toISOString().slice(0, 16).replace('T', ' ') : 'нет вовсе',
        days == null ? '⚠ ни одного будущего матча с котировками'
          : `через ${days.toFixed(1)} сут; в месяце их ${h.priced_month ?? '?'}`,
        days == null ? 'warn' : 'ok');
  } catch (e) {
    row('Котировки: окно экспресса', 'ошибка', String(e).slice(0, 60), 'warn');
  }
}

// ---------------------------------------------------------------- права ---
// ⚠️ КАЖДАЯ СТРОКА ЗДЕСЬ — УСЛОВИЕ ЛИЦЕНЗИИ, А НЕ ОПРЯТНОСТЬ. Снимки с
// Викисклада разрешено показывать (в том числе коммерчески) ровно пока
// названы автор и лицензия. Неподписанный снимок на экране — это не
// «недоделка данных», это использование без выполнения условия.
//
// И это ИМЕННО ЗАМЕР, а не проверка: число неподписанных законно падает до
// нуля и законно растёт, когда сборщик фото добавил новые карточки раньше,
// чем сборщик подписей до них дошёл. Валить прогон на «их стало 40» значило
// бы валить его на нормальную работу конвейера. Смотреть надо на ПОРЯДОК:
// сорок — это ночь работы, четыре тысячи — сборщик подписей не ходит.
async function contentRights() {
  const url = env('VITE_SUPABASE_URL');
  const svc = env('SUPABASE_SERVICE_KEY') || env('SUPABASE_KEY');
  if (!url || !svc) {
    row('Права: подписи к снимкам', 'не измерено', 'нет SUPABASE_KEY', 'skip');
    return;
  }
  const auth = { apikey: svc, Authorization: `Bearer ${svc}`, 'Content-Type': 'application/json' };
  try {
    const t0 = Date.now();
    const r = await fetch(`${url}/rest/v1/rpc/content_rights_gaps`, {
      method: 'POST', headers: auth, body: '{}',
    });
    const ms = Date.now() - t0;
    const gaps = r.ok ? (await r.json().catch(() => [])) : [];

    const sum = (kind) => gaps.filter((g) => g.problem === kind)
                              .reduce((a, g) => a + Number(g.records || 0), 0);

    const unknown = sum('источник не опознан');
    row('Права: записей с неопознанным источником', `${unknown}`,
        unknown === 0 ? 'про всё собранное известно, чьё оно'
                      : '⚠ появился сборщик, не внесённый в content_origin',
        unknown === 0 ? 'ok' : 'warn');

    const unsigned = sum('нет подписи');
    row('Права: снимков без подписи', `${unsigned}`,
        unsigned === 0
          ? 'условие CC BY выполнено у всех'
          : `docs/cards_photo_credits.py доберёт; ${unsigned} файлов ≈ `
            + `${Math.ceil(unsigned / 50)} обращений к Викискладу`,
        unsigned === 0 ? 'ok' : 'warn');

    // ⚠️ ЭТО НЕ ОШИБКА КОНВЕЙЕРА, А РЕШЕНИЕ ВЛАДЕЛЬЦА. Числа здесь означают
    // «показываем, не имея разрешения» — Transfermarkt, ESPN, SoccerWiki,
    // sports.ru. Скрипт их печатает и ничего не требует: убрать, заменить
    // источник или договориться — вопрос не к проверке.
    const unlicensed = sum('нет лицензии');
    row('Права: записей от источников без лицензии', `${unlicensed}`,
        unlicensed === 0 ? 'таких нет'
          : 'решение за человеком: ' + gaps.filter((g) => g.problem === 'нет лицензии')
              .slice(0, 3).map((g) => `${g.source_key} ${g.records}`).join(', '),
        'ok');

    // ⚠️ ЧИСЛО ЗДЕСЬ — ПРО СТОИМОСТЬ, А НЕ ПРО ПОТОЛОК. Ревизия ходит только
    // сервисным ключом, где statement_timeout не стоит: анониму её не отдали
    // ровно потому, что она уже не укладывается в его три секунды (замер
    // 22.09.2026: 5.5 с полного прохода по всем таблицам с контентом).
    // Смотреть стоит на РОСТ: у сервисного ключа потолка нет, но обход,
    // выросший до минуты, съест ночной прогон.
    row('Права: ревизия идёт', `${ms} мс`,
        `обходит все таблицы с контентом; сервисный ключ, потолка нет`
        + (ms > 30000 ? ' — ⚠ но полминуты это уже много' : ''),
        ms > 30000 ? 'warn' : 'ok');
  } catch (e) {
    row('Права: ревизия', 'ошибка', String(e).slice(0, 60), 'warn');
  }
}
