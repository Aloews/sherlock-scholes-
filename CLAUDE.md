...
Расписание клубов ведёт платный провайдер. **Сборных у него нет вообще,
кроме Лиги наций УЕФА** — замер 22.09.2026 по его же справочнику. Остальное
(товарищеские, Лига наций КОНКАКАФ, Кубок Америки, Кубок Африки, шесть
отборов ЧМ, Евро) приходит из ESPN даром через `football-national`.

* **Это единственная функция, которая СОЗДАЁТ строки в `fixtures`.** Для
  клубов правило «расписание ведёт провайдер» осталось прежним.
* **Лига наций УЕФА в реестре ВЫКЛЮЧЕНА строкой с причиной**, а не
  отсутствует. Включить — значит задвоить каждый её матч: провайдер и ESPN
  отдадут одну игру под разными id, и обе строки будут выглядеть правильными.
  Задвоение ловит `check-prod`.
* **Счёт сборных ведёт та же функция**, а не `football-scores-espn`: та
  сводит команды по КЛУБНОМУ словарю псевдонимов, и «Burkina Faso» в нём не
  найдётся никогда. Здесь сопоставление по идентификатору события ESPN.
* **Два режима — ради цены.** Полный обход (13 турниров × 3 месяца = 39
  запросов) идёт дважды в сутки; `scores` каждые пять минут берёт только
  турниры с матчем в окне, а при пустом окне SQL не поднимает контейнер
  вовсе. Полный обход каждые пять минут — 11 232 запроса в сутки к чужому
  бесплатному адресу; за 2304 отсюда уже выброшен раздел «идёт сейчас».
* **Новый турнир сборных заводится в `espn_national_league`**, и тем же
  коммитом — в `KNOWN_SPORT_KEYS` и во все девять локалей. Реестр в базе и
  имена во фронтенде сверяет `test/national_fixtures.test.ts`.

⚠️ Перечень турниров сборных **не выписывается руками нигде, кроме реестра**.
В `check-prod` он был выписан (пять штук), и с приходом ещё девяти две
проверки покраснели ПО КАЛЕНДАРЮ: «6 из 40 матчей с обеими сторонами» в
международный перерыв. Берите его из `espn_national_league`.
...
⚠️ Здесь и в `docs/DEPLOY.md` стояло сперва «семь функций», потом «девять».
Оба раза число устарело молча — ровно как с «остальными семью тестами» ниже,
и второй раз это пережило добавление `football-national`. **Числа функций
здесь больше нет вовсе**: смотрите папку `supabase/functions/`, а сверяет её
со списком в `deploy.yml` тест `test/deploy_functions.test.ts`.
