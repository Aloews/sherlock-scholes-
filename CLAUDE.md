...
Коротко: кнопка прогоняет проверки, выкладывает семь Edge-функций, при
желании переезжает на свой домен и заканчивает `check-prod` уже против нового
адреса. Миграции и фронтенд она не трогает намеренно — причины в документе.

Адрес приложения зашит в трёх местах, и в двух уже перекрывается переменной
(`APP_URL`, `PROD_APP_URL`); CORS у всех семи функций — `*`, то есть переезд
