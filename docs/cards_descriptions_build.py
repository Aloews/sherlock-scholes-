...
# ⚠️ `club_nickname` И `era` УБРАНЫ, И ЭТО НЕ «У НИХ ПЛОХО РЕЗОЛВИТСЯ».
# Метод этого скрипта — «найти статью и взять её преамбулу», а у прозвища
# СВОЕЙ статьи нет: в Википедии есть «Арсенал (футбольный клуб)», но нет
# «Канониров». Резолв поэтому либо не находит ничего, либо находит КЛУБ — и
# тогда карточка-прозвище получает описание другой сущности.
#
# Замер 02.09.2026, настоящей build_for_card на десяти карточках: построено 1,
# отказов 9. И этот единственный «успех» — как раз ошибка: «Сине-бело-голубые»
# получили «Российский футбольный клуб из Санкт-Петербурга». Так же выглядят и
# четыре уже записанные: «Народная команда» — «клуб из Москвы, основан 1922»,
# «Нерадзурри» — «клуб из города Милан».
#
# То есть у этих категорий доля неудач не 90%, а доля ВРЕДНЫХ удач — 100%.
# Прогонять их значит тратить бюджет Wikimedia на то, чтобы иногда испортить
# карточку. Описание прозвищу и эпохе нужно писать иначе (рукой или моделью),
# и это отдельная работа, а не настройка этого конвейера.
NO_OWN_ARTICLE_CATEGORIES = ("club_nickname", "era")
...
#
# `woman` БЫЛА исключена, и условие снятия стояло прямо здесь: «не проверив на
# боевых данных». Проверка выполнена 02.09.2026 — настоящей build_for_card на
# двенадцати карточках, без записи:
#
#   построено описание   10
#   отказов               2   («пустая преамбула ruwiki» — честный отказ,
#                              а не чужая статья)
#
# Прочитано глазами, все десять о том человеке: Бонмати «полузащитник клуба
# „Барселона“ и сборной Испании», Бронз «правый защитник „Челси“», Керр
# «нападающая „Готэм“». Ни тёзок, ни городов, ни поэтов.
#
# Решающий случай — «Марта»: ОДНОСЛОВНОЕ имя, ровно тот страх, из-за которого
# когда-то исключали игроков («Данте» ведёт к поэту). Резолв дал бразильскую
# футболистку, футбольный гард пропустил. То есть оба гарда работают на этой
# категории так же, как на прочих.
#
# Механизм исключения оставлен: он понадобится следующей категории, которую
# захотят придержать до такой же проверки.
EXCLUDED_CATEGORIES = ()
