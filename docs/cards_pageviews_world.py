...
PV_PAUSE = 1.0
WD_BATCH = 50
# ⚠️ ПАЧКА МАЛЕНЬКАЯ НАМЕРЕННО. Двести карточек — это полчаса сбора, и всё это
# время на диске нет ничего: смысл записи «по ходу» теряется целиком, если
# первая запись случается на двухсотой карточке. Двадцать пять — примерно
# минута работы, столько и не жалко потерять на обрыве.
RPC_BATCH = 25
...
def main():
    ...
    ap.add_argument("--minutes", type=int, default=0,
                    help="остановиться через столько минут, дописав собранное. "
                         "0 — без ограничения")
    ...
    written, pending = 0, []
    # ⚠️ СВОЙ ЧАСОВОЙ ПОТОЛОК, А НЕ ЧУЖОЙ SIGTERM. В ночь на 07.09.2026 этот
    # шаг дошёл до 275 карточек из 593 и был убит: exit 143 — это потолок
    # job'а в 330 минут, а не ошибка скрипта. Убитый посреди пачки шаг теряет
    # накопленное и НИЧЕГО не сообщает: в логе просто обрыв. С `--all`
    # запросов на карточку не два, а около тридцати пяти (медиана разделов 44
    # минус уже собранные), то есть примерно тридцать пять секунд на игрока —
    # шестьсот карточек не помещаются в ночь ни при каком раскладе.
    # Здесь скрипт останавливается САМ, дописав пачку и напечатав итог.
    ...
    for n, c in enumerate(resolved, 1):
        if deadline and time.time() > deadline:
            stopped_early = True
            print("  ⏱ отведённые %d мин истекли на %d-й карточке из %d — "
                  "дописываю собранное и выхожу" % (args.minutes, n, len(resolved)),
                  flush=True)
            break
    ...
    print("-" * 70)
    if stopped_early:
        print("⏱ ОСТАНОВЛЕНО ПО ВРЕМЕНИ — это НЕ ошибка и НЕ конец очереди.")
        print("  Уже собранные языки не переспрашиваются, следующая ночь "
