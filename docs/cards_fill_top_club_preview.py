...
def main():
    ...
    with open(OUT_SQL, "w", encoding="utf-8") as fh:
        fh.write(
            "-- ============================================================\n"
            "-- SHERLOCK SCHOLES — cards.top_club / top_minutes / top_league\n"
            "-- Club + minutes + LEAGUE of the player's best season (max\n"
            "-- minutes across the seasons cached from API-Football), shown\n"
            "-- in the quick-game summary and used by the deck league filter.\n"
            "-- Generated by docs/cards_fill_top_club_preview.py — run THIS\n"
            "-- FILE in the Supabase SQL Editor.\n"
            "-- League name is the API-Football label (English, e.g. 'Serie A',\n"
            "-- 'Premier League'); the filter matches on it verbatim.\n"
            "-- pick_random_cards() picks the new columns up automatically.\n"
        ...
        fh.write("ALTER TABLE cards ADD COLUMN IF NOT EXISTS top_league TEXT;\n\n")
        fh.write("BEGIN;\n\n")
        # Guard on top_league IS NULL (not top_club): the earlier run already
        # filled top_club on ~1340 cards, so this backfills the league on those
        # too. top_club/top_minutes are re-set to the same cache-derived value.
        for c, club, stat in matched:
            league = stat.get("league")
            fh.write("UPDATE cards SET top_club = {}, top_minutes = {}, top_league = {} "
                     "WHERE id = '{}' AND top_league IS NULL;  -- {}\n"
                     .format(sql_quote(club), stat["minutes"],
                             sql_quote(league) if league else "NULL",
                             c["id"], c["name"]))
        fh.write("\nCOMMIT;\n")
        if ambiguous:
            fh.write("\n-- AMBIGUOUS (skipped):\n")
