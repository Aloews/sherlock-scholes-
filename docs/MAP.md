...
| `fantasy_position_key`, `fantasy_tactic_points`, `fantasy_tactic_fits`, `fantasy_pick_points`, `fantasy_tactics`, таблицы `fantasy_tactic`/`fantasy_squad_tactic` | `fantasy_tactics.sql` | схема в фэнтези: надбавки за сухарь и голы делятся по линии игрока, схема их взвешивает и требует под себя состав (§4а `docs/FANTASY_AND_MINIGAMES.md`). ⚠️ Очки теперь зависят от того, КТО выбрал карточку, — `fantasy_card_points` (карточка → очки) на этот вопрос больше не отвечает, его заменил `fantasy_pick_points`. ⚠️ У `set_fantasy_squad` ДВЕ перегрузки и ни одной со значением по умолчанию: дефолт сделал бы вызов из четырёх аргументов двусмысленным (`function is not unique`) и уронил бы уже задеплоенный фронтенд |
