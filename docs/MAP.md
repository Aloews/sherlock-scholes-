...
| `digest_topics` | `digest_topics.sql` | горячие **темы**, а не заголовки: заметки — вершины, ребро при трёх общих основах, тема — связная компонента. Работает и через языки на латинице (одно имя собирает Marca, Record и Foot Mercato), и через алфавиты — благодаря `digest_translit`. Горячесть — число **изданий**, а не заметок |
...
| Живой текст русских лент сцепляется транзитивно в один ком на 272 заголовка — это было и до транслитерации, но ком стоял шестым при двух изданиях. Склейка через алфавиты добавляет ему 29 латинских заметок, изданий становится девять, и ком встаёт ПЕРВЫМ, назвавшись одним случайным матчем. Порог ребра 4 ком разбирает (262 → 121) и вместе с ним убивает склейку (языков 4 → 1); центроид вместо «самого раннего» даёт подпись лучше за +5%, но число изданий всё равно врёт. Лечится нетранзитивной кластеризацией | `digest_topics`, `reach`/`comp` |
