...
| `/stream` | `StreamScreen` → `features/stream/` | **Live TV, к игре не относится вовсе** — своё решение с оговорками записано в `docs/ADR/0004`. `VITE_STREAM_URL` отдаёт **каталог из 4081 канала**, а не поток: его разбирает `playlist.ts` (чистый модуль, без импортов приложения), а в плеер идёт адрес ВЫБРАННОГО канала. Показываются только спортивные и только `https` — см. ловушки §9. Над списком **поиск по названию и фильтр «Избранное»** (`filterChannels`), звезда рядом со строкой канала — `toggleFavourite`; избранное живёт в `localStorage` без срока и стоит в порядке ВЫШЕ измеренного здоровья |
...
| 863 юнит-теста зелёные, а ТВ у владельца не работает. Не потому, что тесты плохие, — потому что каждый гонял код против СВОЕГО ЖЕ стенда: плейлист подделан, ответы каналов подделаны, сегменты подделаны. Такой тест не может упасть по причине, по которой ломается приложение. Хуже: ручная проверка останавливалась на первом манифесте. Замер прода по ПОЛНОЙ цепочке HLS показал, что у «Setanta Sports 1/2 HD» master отвечает 200, а variant — **404**, и что «Матч! Премьер» мигает (сегмент 1.3 МБ и 404 с разницей в десять минут). Все три стояли ПЕРВЫМИ, потому что попали в `PINNED` по названию. Лечится двумя вещами: проверкой прода без моков (`scripts/check-prod.mjs`, с отрицательными контролями) и порядком по ИЗМЕРЕННОМУ здоровью вместо ручного списка | `scripts/check-prod.mjs`, `channelCache.markHealth`, `orderChannels` |
...
| Каталог каналов весит **870 910 байт и отдаётся БЕЗ СЖАТИЯ**: заголовка `content-encoding` в ответе нет вовсе, `Accept-Encoding: gzip` релей игнорирует, а `cache-control` у него `max-age=30` — то есть браузер выбрасывает каталог через полминуты и качает заново. На медленном 3G это ~17 секунд молчания ради 34 строк, которые из него в итоге остаются, и игрок справедливо решает, что ТВ не работает. Разобранный список кладётся в `localStorage` и показывается СРАЗУ, сеть идёт следом. Кэшируется РЕЗУЛЬТАТ разбора, а не каталог: 870 КБ — почти весь лимит `localStorage` | `features/stream/channelCache.ts` |
...
| Каждый четвёртый канал в плейлисте не играет, и раньше экран замирал на первом же таком. Замер 25.08.2026 с боевого домена: `Матч! Планета`, `KHL`, `FUTBOL UZ` — **404**; `Arena Sport 3` не отвечает; `NHL`, `Sports Grid`, `UNBEATEN` — 200 **без `access-control-allow-origin`**, то есть играют на iOS (нативный HLS) и не играют на Android (hls.js берёт манифест через XHR). Статического списка «плохих» быть не может: он врал бы половине устройств. Отсев — по факту отказа | `features/stream/order.ts`, `nextAlive` |
...
| Каталог каналов отдан в плеер как поток. `VITE_STREAM_URL` возвращает **M3U-каталог**: 4081 запись `#EXTINF` со ссылками на другие манифесты, и ни одного `#EXT-X-STREAM-INF`, `#EXT-X-TARGETDURATION` или `#EXT-X-VERSION`. `StreamScreen` отдавал этот адрес прямо в `hls.loadSource()` — играть там нечего, и экран `/stream` не мог работать НИ РАЗУ с первого дня. Снаружи это выглядело отказом плеера, а не тем, что ему дали не тот файл | §2, шапка `features/stream/playlist.ts` |
| Избранное отсортировано наравне со здоровьем — и канал, который игрок отметил звездой, уезжал вниз, стоило ему один раз не ответить. Это два РАЗНЫХ утверждения: здоровье — то, что приложение ИЗМЕРИЛО, избранное — то, что игрок СКАЗАЛ, и сказанное человеком не отменяется одним неудачным запросом. Порядок: избранное → здоровье → закреплённые. По той же причине у избранного НЕТ срока годности, в отличие от каталога (24 ч) и здоровья: протухает измерение, а не выбор | `orderChannels`, `readFavourites` в `features/stream/channelCache.ts` |
| Строка поиска обрывала просмотр. `playing` считался от того же отфильтрованного списка, что и рендер, — игрок набирал две буквы, текущий канал выпадал из выборки, и плеер переключался на первый совпавший. Что показано и что играет — разные вопросы: список фильтруется, `playing` берётся из ПОЛНОГО упорядоченного набора | `StreamScreen.tsx`, `ordered` против `list` |
| `http://`-канал в списке Mini App. Приложение открывается по https, браузер режет смешанное содержимое — такой поток не загрузится ни через hls.js, ни через `video.src`, и правкой плеера это не лечится. В боевом плейлисте таких **96 из 127** спортивных: список «всех каналов» был бы на три четверти из заведомо мёртвых строк | `isPlayable`, `features/stream/playlist.ts` |
