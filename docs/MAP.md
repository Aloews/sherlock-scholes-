...
| `digest_topics` | `digest_topics.sql` | горячие **темы**, а не заголовки: заметки — вершины, ребро при трёх общих основах, тема — связная компонента. Работает через языки на латинице (одно имя собирает Marca, Record и Foot Mercato), но не через алфавиты: «Torres» и «Торрес» общих основ не имеют. Горячесть — число **изданий**, а не заметок |
