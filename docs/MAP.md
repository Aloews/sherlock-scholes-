...
| `/arena` | `ArenaScreen` | футбол на двоих **на одном телефоне**: canvas, физика 60 Гц, две площадки управления. Без `PageTransition` — обёртка анимирует transform родителя, и первые кадры уезжали бы вместе с ним. Сетевого варианта нет намеренно, см. §4 |
...
**Сетевой арены не будет**, и это решение, а не пропущенная задача. Синхронная
физика требует обмена 60 раз в секунду при задержке в единицы миллисекунд; в
`docs/VIDEOCHAT_HANDOFF.md` записано измерение этого проекта — 150–300 мс между
континентами. «Один телефон, две команды» — премиса всего приложения, и здесь
она взята буквально.
