...
| таблица `club_crest`, `apply_club_crests()` | `club_crests.sql` | гербы клубов от внешних источников: 993 строки, 711 ESPN и 282 TheSportsDB. ⚠️ **Живёт ОТДЕЛЬНО от `football_club` не для порядка, а потому что иначе не выживает:** ночная пересборка берёт `crest_url` из `cards.photo_url`, то есть из заглавной картинки статьи. Оттуда и были прежние «гербы» — портрет Робби Сэвиджа у «Барнсли», раздевалка у «Арсенала», стадион у «Аякса», мемориальная доска в Швеции у «Абердина». `apply_club_crests()` ставит герб и **снимает** викимедийные фотографии, которым замены не нашлось: пустое место экран рисовать умеет, а фотография стадиона на месте эмблемы читается как поломка |
| `rebuild_clubs_all` и её шаги (`rebuild_football_clubs`, `build_club_aliases`, `merge_seeded_clubs`, `rebuild_club_squads`, `rebuild_club_matches`, `prune_orphan_clubs`) | `football_clubs.sql` | весь конвейер клубов, pg_cron 06:25 UTC. ⚠️ **Порядок — не удобство:** `merge_seeded_clubs` обязан идти ДО составов (состав собирается по `club_key`, и собранный на старый ключ остался бы у удалённой записи), а `prune_orphan_clubs` — ПОСЛЕ матчей, иначе он считает осиротевшим всё подряд, а `apply_club_crests` — САМЫМ ПОСЛЕДНИМ: `rebuild_football_clubs` берёт `crest_url` из `cards.photo_url`, и всё, что поставлено до него, он затирает |
...
| «Герб» брался из `cards.photo_url`, а это заглавная картинка статьи, а не логотип. Из 260 заполненных адресов **143** вели не на эмблему: `Robbie_Savage.jpg` у «Барнсли», `Arsenal_dressing_room_Emirates_Stadium_2024-04-26.jpg` у «Арсенала», `Aberdeen_annedal_minnesplatta.jpg` (доска в шведском Аннедале) у «Абердина». Молча это не ловится: поле заполнено, картинка отдаётся, код 200 — сломано только то, ЧТО на ней | `football_clubs.sql`, `rebuild_football_clubs`; лечится `club_crests.sql` |
| Сведение клуба с источником по равенству `club_norm_key` не работает, и не из-за источников. `club_norm_key` транслитерирует кириллицу, но латинскую диакритику **вырезает целиком**: «1. FC Köln» → `1 k ln`, «IFK Göteborg» → `ifk g teborg`. Плюс источник пишет короче базы («Olympique de Marseille» против «Marseille»). Строгим равенством нашлось 471 совпадение из 1694, токенным счётом с весом IDF — 993 | `football_scraper/club_crests.py` |
| Клуб, у которого в базе нет `name_en`, внешним источникам не найти вовсе. Русское имя транслитерируется по звуку: «Чикаго Файр» → `chikago fayr`, «Рейнджерс» → `reyndzhers`, «Кванджу» → `kvandzhu`. Звучит как оригинал, пишется иначе, общих слов ноль. Это, а не пустота источников, оставило 617 клубов из 1519 без герба — и лечится оно `name_en`, а не порогом | `football_club.name_en`, `football_scraper/club_crests.py` |
