...
| Прогон, намеренно пропустивший ролики, попадает в `channels_silent` — «молчат все одиннадцать» звучит как отказ источника и обесценивает единственный сигнал, ради которого список заведён | `functions/football-digest/index.ts` |
...
| Каждое изменение состава источников требует ПОЛНОГО переноса исходника Edge Function — а перенос делает агент, перепечатывая файл, и однажды уже превратил `ё` в `ᑑ`. Список источников стоило бы держать в таблице: тогда добавление ленты — это INSERT, а не деплой | `functions/football-digest/index.ts`, `FEEDS` / `CHANNELS` |
