...
| таблица `broadcast_rights`, `broadcast_rights_for` | `broadcast_rights.sql` | вещатели по территориям **со сроком от самого источника** (`season_from`/`season_to`) — этим и отличается от того, что `broadcasts` вести отказалась. Территория текстом дословно (`Sub-Saharan Africa`, `Ships and planes` — не страны), код страны отдельной колонкой и только для однозначных. **Оба аргумента функции необязательны** — один путь чтения на оба вопроса («моя страна, все турниры» и «этот турнир, все страны»). Читают `broadcastRightsApi.ts` → `FixtureCard`. ⚠️ Пустой ответ значит «не заявлен», а не «неизвестно» |
