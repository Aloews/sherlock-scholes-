...
| Лента отвечает 200 и выглядит живой, а отдаёт JS-заглушку «Проверка безопасности» вместо XML. Так умер `soccer.ru/rss` — 302 на страницу проверки, ноль `<item>`. Ловится только поимённым `feeds_silent`; у русского осталась одна лента, а громкость при одном источнике тождественно равна единице | `functions/football-digest/index.ts`, `FEEDS` |
