...
| `/fantasy` | `FantasyScreen` | фэнтези: состав из пяти, капитан, **схема**, таблица лиг. **Два разных тура на экране** — заявка на ближайший открытый, таблица по текущему. Схема (`fantasy_tactics.sql`) — единственное настоящее решение здесь: она делит надбавки за сухарь и голы по линии игрока и требует под себя состав, см. §4а `docs/FANTASY_AND_MINIGAMES.md` |
