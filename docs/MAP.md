...
`football_scraper/sports_ru_stats.py` → `player_match_stats`, каждое утро в
04:40 UTC (`.github/workflows/player-stats.yml`). Разбор страниц вынесен в
`scraper/sports_ru.py` — он чистый, и его тесты (`tests/test_sports_ru.py`)
не ходят в сеть.
...
и **проверяется** по шапке страницы: она печатает русское имя, и неверная
догадка отвергается сравнением. На 40 самых известных карточках — 34 попали,
2 страницы отвергнуты проверкой.
...
Что осталось незакрытым: `HomeScreen` на 635 строк с восемью состояниями
`view`, и три безвредных чтения (`inviteApi`, `friend_suggestions`,
`prediction_leaderboard`) — их стоит перенести для единообразия, но ни одно
не вводит игрока в заблуждение.
