...
| `schedule:` не срабатывает на ближайшем сроке сразу после того, как workflow впервые попал в ветку по умолчанию. `player-stats.yml` слился в 04:10 UTC, крон стоял на 04:40 — прогона не было вовсе. Выглядит как сломанный сбор, а на деле расписание ещё не зарегистрировано; отличить можно только запуском через `workflow_dispatch` | `.github/workflows/player-stats.yml` |
