...
def natural_name(label):
    ...
    ⚠️ УТОЧНЕНИЕ В СКОБКАХ СНИМАЕТСЯ ПЕРВЫМ, И ЭТО НЕ КОСМЕТИКА. Первая версия
    разворачивала «Жуан Педро (футболист, 2001)» по запятой ВНУТРИ скобок и
    выдавала «2001) Жуан Педро (футболист» — 44 такие карточки пришлось
    переписывать. Скобка в ярлыке значит «омоним», а не часть имени, и на
    карточке ей делать нечего.
    """
    if not label:
        return label
    # Уточнение отрезается вместе со всем, что за ним: «(футболист, 2001)».
    head_part = label.split("(", 1)[0].strip()
    label = head_part or label
    if label.count(",") != 1:
        return label
