...
--   2026-07-19  Легенды/ballon_dor бэкфилл + колонка descriptions [ПРИМЕНЕНО]
...
-- и 9-арг версию из поздних блоков — файл идёт сверху вниз, финальную версию
-- пересоздаст последний блок
drop function if exists pick_random_cards(int, text[], bigint, text[], text[], text, int, text[], text);
...
-- ============================================================================
-- 2026-07-19 — Pro-чип «Легенды» пуст + колонка описаний  [ПРИМЕНЕНО]
--
...
-- ============================================================================
-- 2026-07-19 — культурная локализация колоды            [ПРИМЕНЕНО]
--
...
-- с default'ами их обслуживает). CREATE OR REPLACE — повторный прогон файла
-- не должен падать с 42723 (случилось 2026-07-19, каюсь).
drop function if exists pick_random_cards(int, text[], bigint, text[], text[], text, int);
...
-- ============================================================================
-- 2026-07-19 — новые категории: дерби / трофеи / эпохи      [ПРИМЕНЕНО]
--
...
-- ============================================================================
-- 2026-07-19 — «слава по языкам» + локальные клубы MX/BR/JP   [ПРИМЕНЕНО]
--
...
-- ============================================================================
-- 2026-07-27 — две порчи данных: чужой name_en и pageviews с чужой статьи
--
-- НАКАТЫВАТЬ НЕЧЕГО: чинят скрипты, блок оставлен как ЖУРНАЛ + VERIFY +
-- ROLLBACK. Исполняемых операторов нет, прогон всего файла его не затрагивает.
--
-- 1) name_en у карточек-людей (docs/cards_name_en_audit.py)
--    ПРИЧИНА: у категорий player/woman/coach/referee/commentator вообще не
--    было P31-гарда в make_card_qid_validator, поэтому односложное имя
--    цепляло статью однофамильца, и её заголовок уезжал в name_en:
--        «Адриан» -> Hadrian (римский император)
--        «Данте»  -> Dante Alighieri (поэт)
--        «Крис»   -> Kris, «Жуан» -> Zhuan (голая транслитерация)
--    Это не косметика: по name_en работает EN-переключатель в игре и по нему
--    же резолвятся фото и просмотры — то есть в футбольную колоду приезжают
--    портрет и популярность императора.
--    ЧИНКА В КОРНЕ: люди теперь обязаны быть Q5, а МОНОНИМ игрока — ещё и
--    футболистом по профессии (P106=Q937857). Составные имена профессией НЕ
--    проверяются: неполный элемент Wikidata иначе потерял бы уже верный резолв.
--    ЧИНКА ДАННЫХ: скрипт правит name_en на enwiki-сайтлинк, найденный по
--    РУССКОМУ имени с гардом; если русское имя не резолвится, а сохранённое
--    значение доказанно принадлежит не футболисту — обнуляет (фронт тогда
--    показывает русское имя, что честнее императора). Во всех остальных
--    случаях не трогает.
--
-- 2) pageviews неигровых карточек (docs/cards_pageviews_refix.py)
--    ПРИЧИНА: --cards-pageviews пробовал ГОЛОЕ название первым, а у клуба это
--    обычно город:
--        «Краснодар» 767074 -> статья о ГОРОДЕ
--        «Астана»    707761 -> столица Казахстана
--        «Оренбург»  580864, «Сатурн» 368643 (планета), «Брест» 255299
--    По этим числам считается порог лёгкой/сложной игры и сортировка «сначала
--    популярные», так что середняк обгонял «Барселону».
--    ЧИНКА В КОРНЕ: режим перешёл на тот же порядок заголовков, что у шага
--    фотографий (клубам и стадионам уточнение «(футбольный клуб)»/«(стадион)»
--    пробуется ПЕРВЫМ) + P31-гард.
--    ЧИНКА ДАННЫХ: если найденная с гардом статья не совпадает с голым
--    названием, просмотры пересчитываются по правильной статье.
--
-- VERIFY name_en (мононимы, у которых значение похоже на чужое):
--   select name, name_en, pageviews
--   from cards
--   where active and category in ('player','woman')
--     and name !~ ' ' and name_en is not null
--   order by coalesce(pageviews,0) desc;
--
-- VERIFY pageviews (клубы: город больше не должен обгонять гранда):
--   select name, pageviews from cards
--   where active and category = 'club'
--   order by pageviews desc nulls last limit 20;
--
-- ROLLBACK: отката «одной кнопкой» нет и быть не должно — оба скрипта
-- ЗАМЕНЯЮТ испорченные значения на снятые из Википедии, а не дописывают
-- новые поля. Прежние значения были неверны (город вместо клуба, император
-- вместо футболиста), возвращать их незачем. Если конкретная карточка
-- исправилась неудачно — правьте её точечно:
--   update cards set name_en = '<верное имя>' where name = '<карточка>';
--   update cards set pageviews = null       where name = '<карточка>';
-- (pageviews = null заставит ночной --cards-pageviews пересобрать счёт.)
-- ============================================================================
